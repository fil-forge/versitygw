...
func NewChunkReader(ctx fiber.Ctx, r io.Reader, authdata AuthData, canonicalString string, cred SigningCred, date time.Time) (io.Reader, error) {
	...
	case payloadTypeStreamingSignedTrailer:
		return NewSignedChunkReader(r, authdata, canonicalString, cred, date, checksumType, true, cLength)
	case payloadTypeStreamingSigned:
		return NewSignedChunkReader(r, authdata, canonicalString, cred, date, "", false, cLength)
